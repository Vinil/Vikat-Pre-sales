...
async function handleChat(request, env, ctx, cfg, cors, user, isAdmin = false) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      /**
       * Whether the web tools have been shed and the turn must start over.
       *
       * Not the same as the schema ladder, which retries the SAME conversation
       * with one tool fewer. A container refusal means the request already
       * carries server-tool results that cannot be continued, so the only way
       * forward is a clean turn without the web tools at all.
       */
      let restartWithoutWeb = false;
      ...
      const degrade = (err) => {
        ...
        if (isContainerRefusal(err)) {
          ...
          // is a far smaller one than the rep getting "Something went wrong"
          // after the searches were already paid for.
          for (const tool of webTools(cfg)) dropped.add(tool.name);
          restartWithoutWeb = true;
          console.error('[chat] container refusal; dropping the web tools and starting the turn over:', err?.message || err);
          return true;
        }
      ...
      const runTurn = async (convo) => {
        ...
        for (;;) {
          try {
            ...
            // here would fail identically. The outer loop rebuilds a clean one.
            if (restartWithoutWeb) return null;
      ...
      try {
        ...
        for (let iteration = 0; iteration < cfg.MAX_TOOL_ITERATIONS; iteration++) {
          ...
          // from the rep's own messages with the web tools gone, and throw
          // away the half-answer the refused attempt had streamed.
          if (final === null) {
            if (!restartWithoutWeb) break;
            restartWithoutWeb = false;
