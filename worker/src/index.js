...
async function handleChat(request, env, ctx, cfg, cors, user, isAdmin = false) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      /**
       * The sandbox the API opened for this turn, if it opened one.
       *
       * The dynamic-filtering web tools run code execution server-side to sift
       * results, and that creates a container. Once it exists, EVERY further
       * request in the same turn has to name it — otherwise the API refuses
       * with "container_id is required when there are pending tool uses
       * generated by code execution with tools", which is what killed a turn
       * that searched the web and then wanted collateral. The searches had
       * already been paid for.
       *
       * Null until a turn actually uses one, and never sent as null: a plain
       * question that never searched has no container and must not claim one.
       */
      ...
      const runTurn = async (convo) => {
        ...
        for (;;) {
          try {
            ...
            // Captured here rather than at the call sites, so no future caller
            // can forget it. `container` is null on any turn that did not run
            // code execution, which is most of them.
