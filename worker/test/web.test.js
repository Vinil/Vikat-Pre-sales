...
import { webTools, sourcesFrom, WEB_SEARCH_TYPE, isContainerRefusal } from '../src/webTools.js';
...
test('the request offers web search', () => {
  const tools = webTools(loadConfig(ENV));
  assert.deepEqual(tools.map((t) => [t.name, t.type]), [['web_search', WEB_SEARCH_TYPE]]);
});

test('the search tool runs no code, so no container can ever be needed', () => {
  // The dynamic-filtering variant sifts results by running code server-side,
  // which opens a container — and every later request in the turn then has to
  // name it, through a parameter that belongs to the beta messages namespace.
  // A turn that searched and then wanted a tool could not continue at all.
  const [search] = webTools(loadConfig(ENV));
  assert.equal(search.type, 'web_search_20250305');
  assert.ok(!/20260209/.test(search.type), 'the filtering variant brings a container with it');
});

test('no beta-only tool type is sent on a stable request', () => {
  // web_fetch_20250910 exists only under client.beta.messages in the installed
  // SDK. Sending a beta type on a stable request is the class of mistake that
  // produced the container failure in the first place.
  const types = webTools(loadConfig(ENV)).map((t) => t.type);
  assert.ok(!types.includes('web_fetch_20250910'), types.join(', '));
  assert.ok(!types.some((t) => t.startsWith('web_fetch')), types.join(', '));
});
...
test('the per-turn cap is on the tool, where the API enforces it', () => {
  ...
  const cfg = loadConfig({ ...ENV, WEB_SEARCH_MAX_USES: '3' });
  const [search] = webTools(cfg);
...
/**
 * The API, stubbed: a search that opens a container, then a tool call.
 *
 * The exact shape that failed in production — "let me research them and pull
 * relevant collateral at the same time".
 */
function searchThenToolApi({ withContainer = true } = {}) {
  const bodies = [];
  let turn = 0;

  const frames = (blocks, stopReason, container) => {
    const start = {
      type: 'message_start',
      message: {
        id: 'm', type: 'message', role: 'assistant', model: 'm', content: [],
        stop_reason: null, stop_sequence: null,
        ...(container ? { container: { id: container, expires_at: '2099-01-01T00:00:00Z' } } : {}),
        usage: { input_tokens: 1, output_tokens: 1 },
      },
    };
    const out = ['event: message_start', `data: ${JSON.stringify(start)}`, ''];
  ...
  const fn = async (url, init) => {
    ...
    const body =
      n === 0
        ? // Searched, and asked for collateral in the same turn. The search
          // opened a sandbox to filter results with.
          frames(
            [{ start: { type: 'tool_use', id: 'tu_1', name: 'find_collateral', input: {} }, deltas: [{ type: 'input_json_delta', partial_json: '{"query":"astec"}' }] }],
            'tool_use',
            withContainer ? 'cnt_astec' : null,
          )
        : frames([{ start: { type: 'text', text: '' }, deltas: [{ type: 'text_delta', text: 'Here is the deck.' }] }], 'end_turn');
...
/**
 * The API, stubbed: refuses the follow-up with the container error, then
 * answers cleanly once the web tools are gone.
 */
function refusingApi() {
  const bodies = [];
  let turn = 0;

  const ok = (blocks, stopReason) => {
    const out = [
      'event: message_start',
      'data: {"type":"message_start","message":{"id":"m","type":"message","role":"assistant","model":"m","content":[],"stop_reason":null,"stop_sequence":null,"usage":{"input_tokens":1,"output_tokens":1}}}',
      '',
    ...
    return {
      ...
      status: 200,
      headers: new Headers({ 'content-type': 'text/event-stream' }),
      body: new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(out.join('\n'))); c.close(); } }),
    };
  };
  ...
  const fn = async (url, init) => {
    ...
    // Searched and asked for collateral, streaming some text on the way.
    if (n === 0) {
      return ok(
        [
          { start: { type: 'text', text: '' }, deltas: [{ type: 'text_delta', text: 'Let me research them' }] },
          { start: { type: 'tool_use', id: 'tu_1', name: 'find_collateral', input: {} }, deltas: [{ type: 'input_json_delta', partial_json: '{"query":"astec"}' }] },
        ],
        'tool_use',
      );
    ...
    // The follow-up, refused — while the web tool is still attached.
    ...
    return ok([{ start: { type: 'text', text: '' }, deltas: [{ type: 'text_delta', text: 'Here is the deck.' }] }], 'end_turn');
  };
...
test('the container refusal is recognised', () => {
  const err = new Error('400 {"type":"error","error":{"type":"invalid_request_error","message":"container_id is required when there are pending tool uses generated by code execution with tools."}}');
  err.status = 400;
  assert.equal(isContainerRefusal(err), true);

  const other = new Error('400 something else');
  other.status = 400;
  assert.equal(isContainerRefusal(other), false, 'a real bug must stay loud');
});

test('a container refusal costs the web tools, not the answer', async () => {
  // Belt and braces. The basic search tool opens no container, so this should
  // never fire — but I have been wrong about this failure twice, and a rep
  // getting a diminished answer beats a rep getting "Something went wrong"
  // after the searches were paid for.
  const stub = refusingApi();
  const { status, text } = await chat(stub, env());

  assert.equal(status, 200);
  assert.match(text, /Here is the deck/, 'the rep still gets an answer');

  const last = stub.bodies.at(-1);
  const names = (last.tools || []).map((t) => t.name);
  assert.ok(!names.includes('web_search'), `web tools should be shed: ${names.join(', ')}`);
  assert.ok(names.includes('find_collateral'), 'and everything else kept');
});

test('the abandoned half-answer is taken off the screen, not stitched to the real one', async () => {
  // The refused attempt had already streamed text. Leaving it there would show
  // the rep a discarded half-sentence running into the answer that replaced it.
  const stub = refusingApi();
  const { text } = await chat(stub, env());

  assert.match(text, /event: reset/, 'the widget has to be told to drop it');
});
