...
test('a turn that searched and then wants a tool names the container it opened', async () => {
  // What a rep saw: seven sources in the rail, then "Something went wrong."
  //   400 invalid_request_error: container_id is required when there are
  //   pending tool uses generated by code execution with tools.
  // The dynamic-filtering web tools run code execution to sift results, and
  // once that sandbox exists every further request in the turn must name it.
  const stub = searchThenToolApi();
  const { status } = await chat(stub, env());

  assert.equal(status, 200);
  assert.equal(stub.bodies.length, 2, 'the turn has to get past the tool call');
  assert.equal(stub.bodies[1].container, 'cnt_astec', 'the follow-up must name the sandbox the search opened');
});

test('a turn that opened no container does not claim one', async () => {
  // Most turns never search. Sending a container they do not have would be a
  // different 400, and a worse one to diagnose.
  const stub = searchThenToolApi({ withContainer: false });
  await chat(stub, env());

  assert.equal(stub.bodies[0].container, undefined);
  assert.equal(stub.bodies[1].container, undefined);
});
